* Compare List methods with JS' array

subtitle:Through dash.el,learn elisp with minor efforts by comparing wiht JS array methods in parallel

You might assume  elisp is hard to learn, yes, but that is a passed old story. Libraries advance with times. In fact, everyone could set out to write elisp even mere JavaScript Foundation.

dash.el is such a library which facilitates elisp and help us manipulatig list effortless.
